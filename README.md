...
```bash
scp -i your-key.pem scripts/bootstrap-ec2.sh ec2-user@<EC2_PUBLIC_IP>:~
...
- **Kubernetes**: replace the SSH deploy step with `kubectl set image
  deployment/densmile-app densmile=$IMAGE_NAME:${{ github.sha }}`, and use
